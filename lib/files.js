...
async function loadConfig(envName, vpconfigName, vpconfigPath, envTemplate) {

    // check if .vpconfig.json exists
    if (!await fs.pathExists(vpconfigPath)) {
        throw new PluginError(PLUGIN_NAME, `Couldn't find a ${vpconfigName} file in your home directory. Please use \'viewport config\' to configure target environments.`)
    }
...
function loadConfigSync(envName, vpconfigName, vpconfigPath, envTemplate) {
...
// resolves glob string to array of filenames, taken relative to CWD
async function resolveGlob(globString) {
